...
export class AuthService {
  constructor(
    ...
    private readonly authUserModel: Model<UserInterface>,
  ...
  async register(registerUserDto: RegisterUserDto): Promise<UserInterface> {
    ...
    registeredUser.password = await bcrypt.hash(registeredUser.password, 10);
  ...
  async validateUser(payload: JwtPayloadInterface): Promise<UserInterface> {
    ...
    const userSignedIn =
      payload.email === user.email && payload._id === user._id.toString();
    if (!userSignedIn) {
      throw new UnauthorizedException();
    }
    return user;
